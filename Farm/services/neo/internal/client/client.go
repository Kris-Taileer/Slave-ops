...
func (nc *Client) UpdateExploit(ctx context.Context, state *epb.ExploitState) (*epb.ExploitState, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("making update exploit request: %w", err)
	}
	return resp.State, nil
}

func (nc *Client) DeleteExploit(ctx context.Context, id string) error {
	req := &epb.DeleteExploitRequest{ExploitId: id}
	if _, err := nc.exploits.DeleteExploit(ctx, req); err != nil {
...
func (nc *Client) SetExploitDisabled(ctx context.Context, id string, disabled bool) error {
	...
	if _, err := nc.exploits.UpdateExploit(ctx, req); err != nil {
		return fmt.Errorf("making update exploit request: %w", err)
	}
	return nil
}
