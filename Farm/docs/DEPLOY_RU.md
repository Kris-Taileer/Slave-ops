...
Для первичной настройки запусти `./farm init`, для повторной — `./farm configure`. Мастер проверяет введённые значения, а токены хранит в игнорируемом `runtime/secrets.env`.
