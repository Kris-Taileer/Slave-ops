...
- **Связанные блоки** (соединены рёбрами `depends_on`) образуют цепочку. Если блок
  вышестоящий не починят и не перезапустят.
- **Авто-цепочка:** когда блок завершается успехом, его прямые потомки (у кого все
  зависимости выполнены) **запускаются сами** — достаточно запустить первый блок, и
  дальше цепочка проходит автоматически. Тот же механизм — fix-and-rerun: правишь
  упавший блок, `Run`, и downstream доезжает сам.
...
- Граф на canvas: узлы можно **таскать мышью** за тело блока — позиция сохраняется
  (`x/y`). Без ручной раскладки блоки выкладываются по топологии автоматически.
- **Соединение курсором:** тянешь за фиолетовый кружок на правом крае блока к другому
  блоку — создаётся связь (со стрелкой направления). **Клик по стрелке** удаляет связь.
- Рёбра зеленеют на успехе предка, краснеют на провале. Цвет левой рамки узла = статус;
  у `running` крутится **спиннер** (виден, пока идёт долгий шаг — напр. docker pull/build).
- Кнопки на блоке: `▶` Run, `■` Stop, `✎` открыть. Двойного клика не нужно — клик по телу
  открывает блок, перетаскивание — двигает.
- Инспектор — просторное **модальное окно** (2 колонки): форма
  (тип/режим/venv/args/timeout/port/зависимости/pass_stdout) + большой CodeMirror-редактор
  (python/shell) и живой вывод. Run/Stop/Restart/Save/Delete в шапке. Закрытие — ✕/Esc/фон.
- Тулбар: «Загрузить примеры», «Сервисы (intro)», «+ Блок», «Запустить всё», «Стоп».
...
- `bandiera-up` — **service**-блок: `docker compose up --build` в foreground
  (`services/bandiera`, app `:8181`, mysql `:3306`). Compose-файлы по умолчанию — режим
  `service`: блок держит стек, `Stop` гасит его. Если pull/build падает (нет доступа к
  registry, сеть легла), compose выходит с ошибкой — блок уходит в **error**, а реальное
  сообщение (напр. `Cannot connect to the Docker daemon` / `Client.Timeout exceeded`)
  видно на блоке и в выводе (ошибка не маскируется).
